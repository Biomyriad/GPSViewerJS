...
cloudDb.getAll = async (offsetId='') => {
  ...
  try {
    ...
    let response = await fetch(`${cloudDb.base_host}/${cloudDb.base_id}/incident%20reports%20and%20observations?pageSize=${offsetId}`, {
...
cloudDb.getOne = async (recId) => {
  let response = await fetch(`${cloudDb.base_host}/${cloudDb.base_id}/`+recId, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${cloudDb.token()}`
